	...
	<div class="app-body" id="view">
		...
		<div class="profile-main padding" id="selectionDepHidden">
			<div class="row details-section">
				<form action="{{route('admin.newsfeeds.store')}}"  method="post" enctype="multipart/form-data">
					...
					<div class="col-md-8">
						<div class="form-group {{($errors->has('title'))?'has-error':''}}">
                           ...
                           <div class="form-group {{($errors->has('description'))?'has-error':''}}">
    							...
    							<textarea rows="8" id="wysiwigEditor" class="form-control user-success" cols="" name="description">{{Request::old('description')}}</textarea>
    							 @if($errors->has('description')) <span class="help-block errorProfilePic">{{$errors->first('description') }}</span> @endif
							</div>
                        <br />
						<div class="form-group">
							<button type="submit" class="btn btn-def"><i class="fa fa-floppy-o"></i>&nbsp;Add News</button> &nbsp;&nbsp;
							<a
