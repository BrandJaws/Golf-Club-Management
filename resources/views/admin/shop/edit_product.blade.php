        ...
        <div class="app-body" id="view">
            ...
            <div class="profile-main padding" id="selectionDepHidden">
                <div class="row details-section">
                    <form action="{{route('admin.shop.update_product',$product->id)}}"  method="post" enctype="multipart/form-data">
                                ...
                                <select name="category_id" id="" class="form-control">
                                    ...
                                    @if($categories && $categories->count()>0)
                                        @foreach($categories as $key=>$category)
                                            <option value="{{$category->id}}" {{(old('category_id') && old('category_id')==$category->id)?'selected="selected"':($product->category_id == $category->id)?'selected="selected"':'' }}>{{$category->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
