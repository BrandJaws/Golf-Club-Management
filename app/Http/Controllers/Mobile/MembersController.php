...
class MembersController extends Controller
{
	...
	public function changePassword(Request $request)
	{
		...
		try {
			$member = Member::where('auth_token', '=', $request->get('auth_token'))->first();
			DB::beginTransaction();
			...
			$member->save();
			DB::commit();
			$this->response = "mobile_password_changed_successuflly";
		} catch (\Exception $e) {
			\DB::rollback();
			\Log::info(__METHOD__, [
				'error' => $e->getMessage()
	...
	public function update(Request $request)
	{
		...
		Auth::user()->populate($request->except('profilePic'))
			->save();
	...
	public function addMemberToFriends(Request $request)
	{
		if (! $request->has('player_id')) {
			...
			if ($member_to_be_friended_id == Auth::user()->id) {
				...
				$member_already_friended = $user->friends()
					->where("id", "=", $member_to_be_friended_id)
					->get()
					->toArray();
	...
	public function removeMemberFromFriends(Request $request)
	{
		if (! $request->has('player_id')) {
			...
			if ($member_to_be_removed_id == Auth::user()->id) {
				...
				$user = Member::find(Auth::user()->id);
				$member_already_friended = $user->friends()
					->where("id", "=", $member_to_be_removed_id)
					->get()
					->toArray();
				if ($member_already_friended == null) {
	...
	public function getFriends()
	{
		...
		$friends = $logged_in_member->friends()
			->select("id", "email", "firstName", "lastName", "phone", "profilePic")
			->get()
			->toArray();
		if ($friends == null) {
	...
	public function updateFriendsGroupInfo(Request $request)
	{
		...
		try {
			...
			if ($request->has('members')) {
				if ((is_array($request->get('members')) && empty($request->get('members'))) || ! is_array($request->get('members'))) {
					$this->error = "no_members_received";
					return $this->response();
				}
				...
				$members = array_unique($members);
				// remove any member that are already in the group
				foreach ($group->members as $groupMember) {
					$memberIndex = array_search($groupMember->id, $members);
					if ($memberIndex > - 1) {
						unset($members[$memberIndex]);
					}
				}
				// Procees to enter new members if
				if (count($members)) {
					$memberModels = Member::whereIn("id", $members)->get();
