        ...
        <div class="app-body" id="vue-container">
            ...
            <div class="profile-main padding" id="selectionDepHidden">
                <div class="row details-section">
                    <form action="{{route('admin.restaurant.store_product')}}"  method="post" enctype="multipart/form-data">
                                ...
                                <select name="restaurant_sub_category_id" id="" class="form-control">
                                    ...
                                    @if($categories && $categories->count()>0)
                                        @foreach($categories as $key=>$category)
                                            <option value="{{$category->id}}" {{(old('restaurant_sub_category_id') && old('restaurant_sub_category_id')==$category->id)?'selected="selected"':($category->id == $selectedCategory ? 'selected="selected"' : '')}}>{{$category->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
